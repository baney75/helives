...
function render(input,seed){
 ...
 const H=s.horizon,X=s.focalX
 parts.push(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1600 1000"><title>${escape(subjects[s.id].title)}</title><defs><linearGradient id="sky" x2="0" y2="1"><stop stop-color="${ink}"/><stop offset=".65" stop-color="${p[1]}"/><stop offset="1" stop-color="${p[2]}"/></linearGradient><radialGradient id="light"><stop stop-color="${bright}" stop-opacity=".42"/><stop offset=".4" stop-color="${gold}" stop-opacity=".12"/><stop offset="1" stop-color="${gold}" stop-opacity="0"/></radialGradient><linearGradient id="water" x2="0" y2="1"><stop stop-color="${p[2]}"/><stop offset="1" stop-color="${ink}"/></linearGradient><linearGradient id="stone" x2="1" y2=".4"><stop stop-color="${p[2]}"/><stop offset=".5" stop-color="${p[1]}"/><stop offset="1" stop-color="${ink}"/></linearGradient><linearGradient id="edge"><stop stop-color="${bright}"/><stop offset="1" stop-color="${p[2]}"/></linearGradient></defs>`)
 ...
 if(s.weather!=='storm'){circle(s.sunX,H-118,s.weather==='stars'?32:66,bright,.65);for(let i=0;i<5;i++)parts.push(`<circle cx="${s.sunX}" cy="${H-118}" r="${74+i*10}" fill="none" stroke="${gold}" stroke-width=".5" opacity="${.22-i*.035}"/>`)}
 ...
 mountain(0,H-100,.8);mountain(1,H-10,1.4)
 ...
 const tree=(x,y,size=1,vine=false)=>{
  ...
  for(const [bx,by] of branches)for(let i=0;i<14;i++){const a=rand()*Math.PI*2,r=rand()*48,l=10+rand()*13,x=bx+Math.cos(a)*r,y=by+Math.sin(a)*r;parts.push(`<path d="M${n(x)} ${n(y)}q${n(-l*.9)} ${n(-l)} ${n(l*.4)} ${n(-l*1.6)}q${n(l*.9)} ${n(l)} ${n(-l*.4)} ${n(l*1.6)}Z" fill="${rand()>.5?p[1]:p[2]}" stroke="${gold}" stroke-width=".5"/>`);if(vine&&i%4===0)circle(x,y+8,3,gold,.8)}
 ...
 const building=(x,y,w,h,tower=false)=>{rect(x,y,w,h,'url(#stone)',gold,.9);bricks(x,y,w,h);arch(x+w*.31,y+h*.35,w*.38,h*.65);for(let i=0;i<(tower?5:8);i++){const ww=w/(tower?5:8);rect(x+i*ww,y-12,ww*.65,15,p[2],gold,.7)}line(x-5,y+h,x+w+5,y+h,bright,1,.7)}
 ...
 switch(s.family){
 case 'sea':
  water(H,1);if(s.id==='exodus')for(const side of [0,1]){const xx=side?1180:-100;path(`M${xx} 1030Q${xx+260} 640 ${xx+80} 410Q${xx+20} 210 ${xx+170} 160Q${xx+290} 200 ${xx+250} 360Q${xx+460} 500 ${xx+390} 1030Z`,p[1],gold,1);for(let i=0;i<65;i++){const off=i*4;path(`M${xx+off} 1040Q${xx+280+off*.2} 630 ${xx+95+off*.4} 400Q${xx+30+off*.5} 240 ${xx+165+off*.4} 195`,'none',i%5===0?bright:gold,.65,.25)}}break
 case 'river':
  water(H+40);path(`M0 ${H+20}Q500 ${H+70} ${X-80} ${H+70}Q${X-260} 700 ${X-260} 810Q${X-600} 840 430 1030H0Z`,p[1],gold,.8);path(`M1600 ${H+20}Q${X+100} ${H+30} ${X+25} ${H+80}Q${X-100} 710 ${X+20} 820Q${X+300} 890 1350 1030H1600Z`,ink,gold,.8);tree(1380,930,1.15);flowers(150,975);break
 case 'garden':case 'harvest':
  mountain(2,H+210,.5);for(let i=0;i<45;i++)path(`M${-500+i*65} 1050Q${X+i*9-300} 740 ${X+i*4-140} ${H+80}`,'none',gold,.6,.25);ground();if(s.id!=='zechariah')tree(X+70,860,1.1,s.detail==='vine');if(s.family==='harvest')for(let i=0;i<95;i++){const xx=rand()*1600,yy=940+rand()*100,ll=50+rand()*150;path(`M${n(xx)} ${n(yy)}q20 ${n(-ll*.6)} 7 ${n(-ll)}`,'none',gold,.8,.65);for(let j=0;j<7;j++){line(xx+7,yy-ll+j*5,xx-2,yy-ll+j*5-8,bright,1.3,.7);line(xx+7,yy-ll+j*5,xx+17,yy-ll+j*5-7,gold,1.3,.8)}}else flowers(400,990);break
 case 'temple':
 ...
 if(s.id==='daniel'){rect(450,810,1040,190,ink);lion(X-130,934,1.6,1);lion(X+240,944,1.2,-1)}
 if(s.id==='zechariah'){
 ...
 if(s.id==='exodus'){
  for(const sx of [-100,1180])for(let i=0;i<55;i++){const a=i*.09;circle(sx+170+Math.cos(a)*52,230+Math.sin(a)*55,1+rand()*2.8,bright,.3+rand()*.3)}
  path(`M${X-22} ${H+6}Q${X-170} 750 ${X-330} 1030H${X+320}Q${X+115} 750 ${X+22} ${H+6}Z`,'url(#stone)',bright,.8)
 ...
 if(s.detail==='stones')for(let i=0;i<7;i++)stone(750+i*70,900+rand()*45,40+rand()*35,45+rand()*55)
...
for(const book of books){const d=directions.find(d=>d.id===book.id);if(!d||!palettes[d.palette])throw Error('Invalid direction');const seed=createHash('sha256').update(book.id).digest().readUInt32LE(0);const svg=optimize(render(d,seed),{multipass:true,plugins:[{name:'preset-default'},{name:'cleanupNumericValues',params:{floatPrecision:2}},{name:'convertPathData',params:{floatPrecision:2}}]}).data;await writeFile(new URL(`public/art/scripture/${book.id}.svg`,root),svg);manifest.push({...book,bookTitle:book.title,...subjects[book.id],family:editorial[book.id]?.family??d.family,file:`${book.id}.svg`,bytes:Buffer.byteLength(svg),sha256:createHash('sha256').update(svg).digest('hex')})}
await writeFile(new URL('docs/art/manifest.json',root),JSON.stringify(manifest,null,2));await writeFile(new URL('src/art/catalog.json',root),JSON.stringify(manifest.map(({id,bookTitle,title,file})=>({id,bookTitle,title,file})),null,2));console.log(`Rendered ${manifest.length} deterministic illustrations; ${Math.round(manifest.reduce((n,a)=>n+a.bytes,0)/1024)} KB total SVG.`)
