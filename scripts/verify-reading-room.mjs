...
try {
 ...
 await p.goto(base+'/?display=1');await p.locator('.is-display .hero-verse').waitFor();await p.getByRole('button',{name:'Exit fullscreen mode',exact:true}).waitFor();await p.clock.runFor(7100)
 ...
 await p.keyboard.press('Escape');assert.equal(await p.locator('.is-display').count(),0);await p.getByRole('button',{name:'Fullscreen mode',exact:true}).waitFor()
 checks.push('Direct fullscreen URL mounts; idle controls fade; pointer reveals; Escape restores home')
 await p.close()
