...
const art=JSON.parse(await readFile('src/art/library.json','utf8'))
...
try{
 ...
 assert.equal(art.length,660);for(const a of [art[0],art[9],art[10],art[art.length-1]]){const response=await p.request.get(base+'/art/scripture/'+a.file);assert.equal(response.status(),200);assert((await response.text()).includes('<svg'))}
 ...
 await p.keyboard.press('ArrowRight');assert.equal(await p.getByLabel('Choose a book artwork').inputValue(),'exodus');assert.equal(await p.locator('.art-scene-indicator').textContent(),'Scene 2 of 10');await p.keyboard.press('Escape')
 ...
 checks.push('660-scene manifest boundaries serve; 66-book selector and scene navigation work; backdrop cleans old layer; Scripture pause stops camera; no audio requests or playback before opt-in')
 ...
 await sheet.close();assert.deepEqual(errors,[]);checks.push('375px home and viewer fit; HD TV render; reduced motion stops artwork; collection boundary contact sheets capture representative scenes')
